...
def search_tikz_examples(query: str, use_bigquery: bool = True, k: int = 5) -> List[Dict[str, Any]]:
    """
    Unified search interface for TikZ examples using BigQuery or local KB.
    ...
    # Try our own local semantic search as final fallback
    try:
        import asyncio
...
async def search_tikz_examples_async(query: str, use_bigquery: bool = True, k: int = 5) -> List[Dict[str, Any]]:
    """
    Async version of unified search interface for TikZ examples.
    ...
    if use_bigquery:
        try:
            ...
            logger.info("Attempting BigQuery search...")
            # BigQuery operations are typically sync, so wrap in thread
            def bigquery_search():
                kb_tool = BigQueryKBTool()
                return kb_tool.semantic_search(query)
            
            results = await asyncio.to_thread(bigquery_search)
