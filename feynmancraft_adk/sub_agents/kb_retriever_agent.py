...
def search_bigquery_tikz_examples(query: str) -> List[Dict[str, Any]]:
    """
    Search for TikZ examples using BigQuery (production mode).
    
    Args:
        query: Natural language query about the Feynman diagram
        
    Returns:
        List of relevant TikZ examples from BigQuery
    """
    try:
        return search_tikz_examples(query, use_bigquery=True, k=5)
    except Exception as e:
        logger.error(f"Error in BigQuery search: {e}")
...
KBRetrieverAgent = Agent(
    ...
    description="Retrieves relevant TikZ examples from BigQuery or local knowledge base using semantic/vector search. Uses centralized tools for all search operations.",
